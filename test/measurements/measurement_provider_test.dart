...
import 'package:werkaut/models/measurements/measurement_entry.dart';
...
// class MockwerkautBaseProvider extends Mock implements werkautBaseProvider {}
...
void main() {
  ...
  setUp(() {
    ...
    measurementProvider = MeasurementProvider(mockwerkautBaseProvider);

    when(mockwerkautBaseProvider.makeUrl(any)).thenReturn(tCategoryUri);
    when(mockwerkautBaseProvider.makeUrl(any, id: anyNamed('id'))).thenReturn(tCategoryUri);
    when(mockwerkautBaseProvider.fetch(any))
        .thenAnswer((realInvocation) => Future.value(tMeasurementCategoriesMap));

    when(mockwerkautBaseProvider.makeUrl(entryUrl, query: anyNamed('query')))
        .thenReturn(tCategoryEntriesUri);
    when(mockwerkautBaseProvider.makeUrl(entryUrl, id: anyNamed('id'), query: anyNamed('query')))
        .thenReturn(tCategoryEntriesUri);
    when(mockwerkautBaseProvider.fetch(tCategoryEntriesUri))
  ...
  group('fetchAndSetCategories()', () {
    test('should make a request url', () async {
      ...
      // assert
      verify(mockwerkautBaseProvider.makeUrl(categoryUrl));
    });
    ...
    test('should fetch data from api', () async {
      ...
      // assert
      verify(mockwerkautBaseProvider.fetch(tCategoryUri));
    });
  ...
  group('fetchAndSetCategoryEntries()', () {
    ...
    test('should make a uri from a category id', () async {
      ...
      // assert
      verify(mockwerkautBaseProvider.makeUrl(entryUrl, query: {'category': tCategoryId.toString()}));
    });
    ...
    test('should fetch categories entries for id', () async {
      ...
      // assert
      verify(mockwerkautBaseProvider.fetch(tCategoryEntriesUri));
    });
  ...
  group('addCategory()', () {
    ...
    setUp(() {
      when(mockwerkautBaseProvider.post(any, any))
          .thenAnswer((realInvocation) => Future.value(tMeasurementCategoryMap));
    });
    ...
    test("should post the MeasurementCategorie's Map", () async {
      ...
      // assert
      verify(mockwerkautBaseProvider.post(tMeasurementCategoryMapWithoutId, tCategoryUri));
    });
  ...
  group('deleteCategory()', () {
    ...
    test(
        ...
        () async {
      // arrange
      when(mockwerkautBaseProvider.deleteRequest(any, any))
      ...
      // assert
      verify(mockwerkautBaseProvider.deleteRequest('measurement-category', tCategoryId));
    ...
    test(
        'should re-add the "removed" MeasurementCategory and relay the exception on werkautHttpException',
        () async {
      // arrange
      when(mockwerkautBaseProvider.deleteRequest(any, any)).thenThrow(werkautHttpException('{}'));

      // act & assert
      expect(() async => measurementProvider.deleteCategory(tCategoryId),
          throwsA(isA<werkautHttpException>()));
      expect(measurementProvider.categories, tMeasurementCategories);
    });
  });
  ...
  group('editCategory()', () {
    ...
    setUp(() async {
      when(mockwerkautBaseProvider.patch(any, any))
    ...
    test('should call api to patch the category', () async {
      ...
      // assert
      verify(mockwerkautBaseProvider.patch(tCategoryMapEditedToJson, tCategoryUri));
    });

    test('should keep categories list as is on werkautHttpException', () {
      // arrange
      when(mockwerkautBaseProvider.patch(any, any)).thenThrow(werkautHttpException('{}'));
  ...
  group('addEntry()', () {
    ...
    test('should make the post url', () async {
      ...
      // assert
      verify(mockwerkautBaseProvider.makeUrl(entryUrl));
    });
    ...
    test('should post the MeasurementEntryMap', () async {
      ...
      // assert
      verify(mockwerkautBaseProvider.post(measurementEntryMapWithoutId, tCategoryEntriesUri));
    });
  ...
  group('deleteEntry()', () {
    ...
    test('should call the api to remove the MeasurementEntry', () async {
      ...
      // assert
      verify(mockwerkautBaseProvider.deleteRequest(entryUrl, tEntryId));
    });

    test(
        'should re-add the "removed" MeasurementEntry and throw a werkautHttpException if the api call fails',
        () async {
      // arrange
      final List<MeasurementCategory> tMeasurementCategories = [
        MeasurementCategory(id: 1, name: 'Strength', unit: 'kN', entries: [
  ...
  group('editEntry()', () {
    ...
    test('should call api to patch the entry', () async {
      ...
      // assert
      verify(mockwerkautBaseProvider.patch(tEntryMapEdited, tCategoryEntriesUri));
    });
  });
}
